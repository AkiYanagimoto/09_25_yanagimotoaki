...
//2. データ表示SQL作成
$sql = 'SELECT*FROM php03_user_table';
...
if ($status == false) {
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $view .= '<p>' . "ID :" . "  " . $result['lid'] . '</p>';
        $view .= '<p>' . "PASSWORD :" . "  " . $result['lpw'] . '</p>';
        if ($result['kanri_ flg'] == 0) {
            $view .= '<p>' . "ADMINISTRATOR :" . "  " . 'YES' . '</p>';
        } else ($result['kanri_flg'] != 0){
            $view .= '<p>' . "ADMINISTRATOR :" . "  " . 'NO' . '</p>'};
        if ($result['life_flg'] == 0) {
            $view .= '<p>' . "ACTIVE :" . "  " . 'YES' . '</p>';
        } else ($result['life_flg'] != 0){
            $view .= '<p>' . "ACTIVE :" . "  " . 'NO' . '</p>'};
